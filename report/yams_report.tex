...
was required to get webpages to display in a browser: GET, POST, Expect, and
...
using the string operations library to modify the URI into a useful file path.
The processing testing file is \texttt{mips/test\_file.asm}.
...
We have implemented an HTTP server, so there isn’t really a “user interface” per
se. To compile Mars with our implemented syscalls, run make from the project
...
frontend interface for the YAMS server's Brainfuck\cite{Mpreu/preller}

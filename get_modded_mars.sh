...
# Create a file so we have a reference timestamp, so we can compile only the
# files our patch changes.
touch $TOUCHED_FILE
rm -rf $MARS_DIR
mkdir -p $MARS_DIR
cd $MARS_DIR
...
## This patch contains all of the source file changes needed to mod MARS.
patch -p1 < ../$PATCH_NAME
cd ..
